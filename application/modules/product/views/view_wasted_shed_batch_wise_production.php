        ...
        <div class="col-md-12">
            <section class="panel">
                <header class="panel-heading">
                    <i class="fa fa-user"></i> <?= lang('production_wasted_details'); ?>
                </header>
                    ...
                    <div class="adv-table editable-table ">
                                        ...
                                        <div class="col-xs-4">
                                            <table>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="table__padding">
                                                        <p>: <?= $settings->currency; ?> <?= $productById->pr_selling_price; ?>
                                                        </p>
                                                    </td>
                                                </tr>
                                                <tr>
                        ...
                        <table class="table table-striped table-hover table-bordered" id="editable-sample">
                            ...
                            <tbody>
                                <?php
                                $serial = 0;
                                foreach ($wastedProductions as $list) {
                                    $serial++;
                                ?>
                                    <tr class="">
                                        <td><?= $serial ?></td>
                                        <td><?= date("$settings->date_format", strtotime($list->prw_date)); ?></td>
                                        <td><?= $this->shed_model->getShedById($list->prw_shed_id)->sh_no ?>: <?= $this->shed_model->getShedById($list->prw_shed_id)->sh_title ?> </td>
                                        <td><?= $this->purchase_model->getAssignedSummaryDataByShedAndBatchId($list->prw_shed_id, $list->prw_batch_id)->lshs_batch_id ?>: <?= $this->purchase_model->getAssignedSummaryDataByShedAndBatchId($list->prw_shed_id, $list->prw_batch_id)->lshs_batch_title ?> </td>
                                        <td> <?= $list->prw_wasted_quantity; ?></td>
                                        <td> <?= $list->prw_description; ?></td>
                                        <td>
                                            <button type="button" class="button button-warning editButton" data-toggle="modal" data-date="<?= date("$settings->date_format", strtotime($list->prw_date)); ?>" data-unit="<?php if ($unit) {
                                                                                                                                                                                                                                echo $unit->un_name;
                                                                                                                                                                                                                            } ?>" data-stock-quantity="<?= $SbWiseStockProduction; ?>" data-id="<?= $list->prw_id; ?>" data-product-assign-id="<?= $list->prw_pra_id ?>" data-product-name="<?= $productById->pr_name ?>" data-category-name="<?= $productCategory; ?>" data-date="<?= date("$settings->date_format", strtotime($list->prw_date)); ?>" data-shed-id="<?= $list->prw_shed_id; ?>" data-batch-id="<?= $this->purchase_model->getAssignedSummaryDataByShedAndBatchId($list->prw_shed_id, $list->prw_batch_id)->lshs_batch_id ?>" data-quantity="<?= $list->prw_wasted_quantity; ?>" data-description="<?= $list->prw_description; ?>" data-shed-value="<?= $this->shed_model->getShedById($list->prw_shed_id)->sh_no ?>: <?= $this->shed_model->getShedById($list->prw_shed_id)->sh_title ?>" data-batch-value="<?= $this->purchase_model->getAssignedSummaryDataByShedAndBatchId($list->prw_shed_id, $list->prw_batch_id)->lshs_batch_id ?>: <?= $this->purchase_model->getAssignedSummaryDataByShedAndBatchId($list->prw_shed_id, $list->prw_batch_id)->lshs_batch_title ?>"><i class="fas fa-edit"></i> <?php echo lang('edit'); ?></button>
                                            <form action="<?php echo base_url('product/deleteProductWasted'); ?>" method="post" style="display:inline" onsubmit="return confirm('<?= lang('are_you_sure_want_to_delete_this_item'); ?>');">
                                                <input type="hidden" name="prw_id" value="<?= $list->prw_id; ?>">
                                                <input type="hidden" name="prw_pra_id" value="<?= $list->prw_pra_id; ?>">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-info">
                ...
                <h4 class="modal-title"><strong><i class="fa fa-edit"></i> <?= lang('edit_wasted_production'); ?></strong></h4>
            </div>
            <div class="modal-body" style="height:100%;">
                <form role="form" id="productionEditForm" action="<?php echo base_url('product/updateProductionWasted'); ?>" method="post" enctype="multipart/form-data">
                    <div class="row">
                    ...
                    <div class="form-group">
                        <label for="exampleInputEmail1"><?= lang('production_date'); ?></label>
                        <input type="text" class="form-control datepicker" name="prw_date" id="wastedDate" value='' placeholder="" required>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1"><?php echo lang('description'); ?></label>
                        <textarea name="prw_description" class="form-control" id="description" rows="5" placeholder="Enter description" style="height: auto !important;"></textarea>
                    </div>
                    <input type="hidden" name="prw_pra_id" value='' id="productAssignId">
                    <input type="hidden" name="prw_id" value='' id="productionId">
                    <section class="">
