    ...
    <section class="wrapper site-min-height">
        ...
        <section class="panel">
            <header class="panel-heading bg-info">
                <i class="fas fa-edit"></i> <?= lang('update_product_sale'); ?>
            </header>
                        ...
                        <div class="col-md-12">
                            <section class="panel">
                                <div class="panel-body">
                                    ...
                                    <form role="form" action="<?php echo base_url('') ?>sale/updateProductSale" method="post" id="formId" enctype="multipart/form-data">
                                        ...
                                        <div class="row">
                                                    ...
                                                    <div class="col-md-12">
                                                        <table class="table table-bordered data-table">
                                                            ...
                                                            <tbody>
                                                                ...
                                                                ?>
                                                                    <tr class="session_data_store_class" data-prs_pra_id="<?= $summary_id->prsv_pra_id ?>" data-product-sale-id="<?= $summary_id->prsv_product_id ?>" data-prs-unit-price="<?= $summary_id->prsv_unit_price ?>" data-prs-quantity="<?= $summary_id->prsv_quantity ?>" data-prs-discount="<?= $summary_id->prsv_discount ?>" data-prs-total="<?= $summary_id->prsv_total ?>">
                                                                        <td>
                                                                            <?= $this->product_model->getProductById($summary_id->prsv_product_id)->pr_name; ?> <input type="hidden" name="prs_pra_id[]" value="<?= $summary_id->prsv_pra_id ?>">
                                                                            <?php
                                                                            $assignProductById = $this->product_model->getProductAssignById($summary_id->prsv_pra_id);
                                                                            echo $this->shed_model->getShedById($assignProductById->pra_shed_id)->sh_no ?>: <?= $this->shed_model->getShedById($assignProductById->pra_shed_id)->sh_title ?> -
                                                                            <?= $this->purchase_model->getAssignedSummaryDataByShedAndBatchId($assignProductById->pra_shed_id, $assignProductById->pra_batch_id)->lshs_batch_id ?>: <?= $this->purchase_model->getAssignedSummaryDataByShedAndBatchId($assignProductById->pra_shed_id, $assignProductById->pra_batch_id)->lshs_batch_title ?>
                                                                        </td>

                                                                        <td> <?= $summary_id->prsv_unit_price ?><input type="hidden" name="prs_unit_price[]" value="<?= $summary_id->prsv_unit_price ?>"></td>
                                                                        <td> <?= $summary_id->prsv_quantity ?><input type="hidden" name="prs_quantity[]" value="<?= $summary_id->prsv_quantity ?>"></td>
                                                                        <td> <?= $summary_id->prsv_discount ?><input type="hidden" name="prs_discount[]" value="<?= $summary_id->prsv_discount ?>"></td>
                                                                        <td> <?= $summary_id->prsv_total ?><input type="hidden" class="for_sum_sub_total" name="prs_total[]" value="<?= $summary_id->prsv_total ?>"></td>
                                                                        <td>
                                                                            <button type="button" class='button button-warning dynamic-product-edit display_hide'><i class="fas fa-edit"></i></button>
                                                                            <button type="button" class='button button-danger btn-delete' data-prs_pra_id="<?= $summary_id->prsv_pra_id ?>" data-quantity="<?= $summary_id->prsv_quantity ?>"><i class="fas fa-trash"></i></button>
                                                                        </td>
                                                                    </tr>
                                                                <?php } ?>
                                                            </tbody>
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control m-bot15" name="prs_c_id" value='' required>
                                                        <option value=""><?= lang('please_select_client'); ?></option>
                                                        <?php foreach ($clients->result() as $client) { ?>
                                                            <option value="<?php echo $client->c_id; ?>" <?php
                                                                                                            if (!empty($productSaleById))
                                                                                                                if ($productSaleById->prss_c_id == $client->c_id) {
                                                                                                                    echo "selected";
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control datepicker" name="prs_date" id="" value='<?php if (!empty($productSaleById)) {
                                                                                                                                        echo date("$settings->date_format", strtotime($productSaleById->prss_date));
                                                                                                                                    } ?>' placeholder="" autocomplete="off" required>
                                                </div>
                                                <div class="form-group">
                                                    <label for="exampleInputEmail1"> <?php echo lang('sub_total'); ?><span class="text-danger">*</span></label>
                                                    <input type="text" class="form-control calculate_sub_total input__number readonly" name="prs_sub_total" id="sub_total" value='<?php if (!empty($productSaleById)) {
                                                                                                                                                                                        echo $productSaleById->prss_sub_total;
                                                                                                                                                                                    } ?>' placeholder="" required>
                                                </div>
                                                <div class="form-group display_hide">
                                                    <label for="exampleInputEmail1"> <?php echo lang('discount'); ?> </label>
                                                    <input type="text" class="form-control calculate_grand_discount input__number" name="prs_grand_discount" id="discount" value='<?php if (!empty($productSaleById)) {
                                                                                                                                                                                        echo $productSaleById->prss_grand_discount;
                                                                                                                                                                                    } ?>' placeholder="">
                                                </div>
                                                <div class="form-group display_hide">
                                                    <label for="exampleInputEmail1"> <?php echo lang('amount_payable'); ?><span class="text-danger">*</span></label>
                                                    <input type="text" class="form-control calculate_grand_total readonly" name="prs_grand_total" id="grand_total" value='<?php if (!empty($productSaleById)) {
                                                                                                                                                                                echo $productSaleById->prss_grand_total;
                                                                                                                                                                            } ?>' placeholder="" required>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-md-12 note">
                                                        <textarea class="form-control" name="prs_note" value="" rows="5" cols="10" style="height: auto !important;"> <?php if (!empty($productSaleById)) {
                                                                                                                                                                            echo $productSaleById->prss_note;
                                                                                                                                                                        } ?></textarea>
                                                    </div>
                                                </div>
                                                <input type="hidden" name="prss_id" value='<?php if (!empty($productSaleById)) {
                                                                                                echo $productSaleById->prss_id;
                                                                                            } ?>'>
                                                <div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(".form").click(function(e) {
            ...
            // Store prs_pra_id id in session 
            sessionStorage.setItem(prs_pra_id, prs_pra_id);

            // Store prs_pra_id id in session 
            let dynamic_prs_pra_id = prs_pra_id;
            sessionStorage.setItem(dynamic_prs_pra_id, prs_pra_id);
            if (sessionStorage.getItem("quantity_" + dynamic_prs_pra_id)) {
                let new_session = sessionStorage.getItem("quantity_" + dynamic_prs_pra_id);
                sessionStorage.setItem("quantity_" + dynamic_prs_pra_id, parseInt(new_session) - parseInt(prs_quantity));
            } else {
                sessionStorage.setItem("quantity_" + dynamic_prs_pra_id, -parseInt(prs_quantity));
            }
        ...
        $("body").on("click", ".btn-delete", function() {
            ...
            if (sessionStorage.getItem("quantity_" + dynamic_prs_pra_id)) {
                let new_session = sessionStorage.getItem("quantity_" + dynamic_prs_pra_id);
                sessionStorage.setItem("quantity_" + dynamic_prs_pra_id, parseInt(new_session) + parseInt(quantity));
            } else {
                sessionStorage.setItem("quantity_" + prs_pra_id, quantity);
            }
        ...
        $("body").on("click", ".dynamic-product-edit", function() {
            ...
            var prs_pra_id = $(this).parents("tr").attr('data-product-sale-id');
            var prs_unit_price = $(this).parents("tr").attr('data-prs-unit-price');
            var prs_quantity = $(this).parents("tr").attr('data-prs-quantity');
            var prs_discount = $(this).parents("tr").attr('data-prs-discount');
            var prs_total = $(this).parents("tr").attr('data-prs-total');

            $(this).parents("tr").find("td:eq(0)").html(`<div class="form-group col-sm-2" style="width: 100%">
                                                            <select name="prs_pra_id" id="${prs_product_dynamic}" class="form-control js-example-basic-single selectedValueEdit" data-id='${prs_pra_id}' placeholder="" style="width: 100%;">
                                                                <option value=""><?= lang('please_select_product_from_shed_and_batch'); ?></option>
                                                                    <?php if (!empty($assignedProducts)) foreach ($assignedProducts as $product) {
                                                                    ?>
                                                                        <option value="<?php echo $product->pra_pr_id ?>">
            ...
            $(this).parents("tr").find("td:eq(1)").html('<input class="form-control new_prs_unit_price" name="prs_unit_price" value="' + prs_unit_price + '" style="width: 100%">');
            $(this).parents("tr").find("td:eq(2)").html('<input class="form-control new_prs_quantity" name="prs_quantity" value="' + prs_quantity + '" style="width: 100%">');
            $(this).parents("tr").find("td:eq(3)").html('<input class="form-control new_prs_discount" name="prs_discount" value="' + prs_discount + '" style="width: 100%">');
            $(this).parents("tr").find("td:eq(4)").html('<input class="form-control new_prs_total" name="prs_total" value="' + prs_total + '" style="width: 100%" readonly>');
            ...
            $(this).parents("tr").find("td:eq(5)").prepend("<button class='button button-info btn-xs dynamic-product-update' style='width: auto;'><?= lang('update'); ?></button>");
            $(this).hide();
        });
        ...
        $("#prs_pra_id").change(function() {
            ...
            $.ajax({
                ...
                let session_quantity = sessionStorage.getItem("quantity_" + prs_pra_id);

                // alert(session_quantity);
                if (session_quantity) {
                    if (session_quantity > 0) {
                        response.product_stock += parseInt(session_quantity);
                    }
                }
                ...
                getAlertData(response.product_stock);
            });
        });
